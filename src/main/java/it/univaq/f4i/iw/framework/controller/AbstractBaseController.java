...
public abstract class AbstractBaseController extends HttpServlet {
    ...
    private void processBaseRequest(HttpServletRequest request, HttpServletResponse response) {
        ...
        try (DataLayer datalayer = createDataLayer((DataSource) getServletContext().getAttribute("datasource"))) {
            datalayer.init();
            initRequest(sanitizedRequest, datalayer);
            if (checkAccess(sanitizedRequest, response)) {
                accessCheckSuccessful(sanitizedRequest, response);
                processRequest(sanitizedRequest, response);
            } else {
                accessCheckFailed(sanitizedRequest, response);
            }
        } catch (Exception ex) {
            handleError(ex, sanitizedRequest, response);
        }
    }
